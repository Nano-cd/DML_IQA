...
class CNNIQAnet(nn.Module):
    ...
    def forward(self, x):
        am = x[0].view(-1, x[0].size(-3), x[0].size(-2), x[0].size(-1))  # 32�32�G
        al = x[1].view(-1, x[1].size(-3), x[1].size(-2), x[1].size(-1))  # 32�32ؑ����
        ai = x[2].view(-1, x[2].size(-3), x[2].size(-2), x[2].size(-1))  # ݻ�o
